...
- **Last completed task:** MEXT-014 — Multi-object-header Class-0 parse harden
- **Last checkpoint commit:** `73e02cd` (MEXT-001..003 checkpoint) — pushed to origin/main
- **Current task:** none (idle) — next READY is MEXT-016
...
**MEXT-016 — Qualifier allow-list vs 1815 for v0 path**. Ensure Read/Operate request builders use only the IEEE 1815 standard qualifier codes (no v0-private qualifier shortcuts) so requests are interoperable with real outstations. Add golden request vectors and assert the wire qualifier bytes match 1815. verify-mvp.sh must stay green.
...
1. Read `active_work/MEXT_MASTER_ROADMAP.md` (MEXT-016).
2. Implement **MEXT-016** (Qualifier allow-list vs 1815 for v0 path).
3. Checkpoint after MEXT-016 (1 task) — run go test ./... + verify-mvp.sh, commit, push.
...
NEXT TASK: MEXT-016 — Qualifier allow-list vs 1815 for v0 path
```
